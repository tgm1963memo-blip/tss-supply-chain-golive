...
REM Run daily master/current Express read-only sync.
...
set "HH=%HH: =0%"
set "LOG_FILE=logs\master_daily_sync_%YYYY%%MM%%DD%_%HH%%MI%%SS%.log"

echo ================================================== >> "%LOG_FILE%"
echo Master Daily Sync started at %date% %time% >> "%LOG_FILE%"
...
%PYTHON_EXE% sync_express_readonly.py --active-rooms --table STMAS.DBF >> "%LOG_FILE%" 2>&1
%PYTHON_EXE% sync_express_readonly.py --active-rooms --table ARMAS.DBF >> "%LOG_FILE%" 2>&1
...
echo ================================================== >> "%LOG_FILE%"
echo Master Daily Sync finished at %date% %time% >> "%LOG_FILE%"
echo ================================================== >> "%LOG_FILE%"
